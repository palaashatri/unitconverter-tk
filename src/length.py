...
def LengthConverter(parent: tk.Widget, on_status=None, state=None) -> ttk.Frame:
    ids = {
        "Nautical Miles": "nmi",
        "Miles": "mi",
        "Yards": "yd",
    ...
    converting = False  # Prevent recursive conversion
    ...
    def callback(event=None):
        ...
        try:
            frm = ids[in_unit.get()]
            to = ids[out_unit.get()]
            result = convert_length.convert(amt, frm, to)
    ...
    ttk.Label(frame, text="Length Converter", style="Header.TLabel").grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 12))
    ...
    # Keyboard shortcuts
    frame.bind_all("<Control-c>", lambda e: copy_result() if out_amt.get() else None)
    frame.bind_all("<Command-c>", lambda e: copy_result() if out_amt.get() else None)
