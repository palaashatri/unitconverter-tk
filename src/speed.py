...
from . import convert_speed
from . import utils


def SpeedConverter(parent: tk.Widget, on_status=None, state=None) -> ttk.Frame:
    ids = {
        "Meters/sec": "mps",
        "Kilometers/hr": "kph",
        "Miles/hr": "mph",
    ...
    def callback(event=None):
        ...
        try:
            frm = ids[in_unit.get()]
            to = ids[out_unit.get()]
            result = convert_speed.convert(amt, frm, to)
    ...
    ttk.Label(frame, text="Speed Converter", style="Header.TLabel").grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 12))
