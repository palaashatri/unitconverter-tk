...
from . import convert_time
from . import utils


def TimeConverter(parent: tk.Widget, on_status=None, state=None) -> ttk.Frame:
    ids = {
        "Microsecond": "us",
        "Millisecond": "ms",
        "Second": "sec",
    ...
    def callback(event=None):
        ...
        try:
            frm = ids[in_unit.get()]
            to = ids[out_unit.get()]
            result = convert_time.convert(amt, frm, to)
    ...
    ttk.Label(frame, text="Time Converter", style="Header.TLabel").grid(row=0, column=0, columnspan=3, sticky="w", pady=(0, 12))
